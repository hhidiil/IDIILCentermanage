    ...
    <div class="studentManage_section">
      ...
      <section class="select_section">
        <el-row>
          ...
          <div v-if="MaCenter">
            <span style="margin-left: 20px;font-weight: bold">选择分中心：</span>
            <el-select v-model="centerValue" placeholder="请选择" @change="selectCenter">
              <el-option
          ...
          <span v-if="SubCenter" style="font-weight: bold">中心号：{{msg}}</span>
      ...
      <section class="section_table">
        ...
        <el-dialog
          :title="aFlag ? '添加用户':'修改信息' "
          :show-close="modalClickOther"
          :visible.sync="dialogVisible"
          :closeOnClickModal="modalClickOther"
          width="40%">
          <el-form :model="ruleForm" status-icon :rules="rules" ref="aForm" label-width="80px" class="demo-ruleForm">
...
<script type="text/ecmascript-6">
  ...
  import {getAllClassesOfCenter,getAllStudentOfClass,getAllManagerList} from './../api/manage'
...
//        console.log( centerId )
        this.ruleForm.centerId = this.centerId;
        let result1 = await getAllClassesOfCenter({centerId:this.centerId});
      ...
      async deleteSelection() {
        ...
        let row = this.$refs.multipleTable.selection;
        console.log( row )
        row.map( async(item,index)=>{
          console.log(item)
          let result = await deleteStudentUser({userId:item.studentId});
          if(result.code == 200){
            this.$message({message: '删除成功！',type:'success'});
          }
        })
